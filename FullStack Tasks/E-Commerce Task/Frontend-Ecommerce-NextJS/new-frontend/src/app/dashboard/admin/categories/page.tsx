...
import { Loader2, Plus, Edit, Trash2, Tag, X, Save, AlertCircle } from 'lucide-react'
...
export default function AdminCategoriesPage() {
  ...
  const isDuplicateCategory = (name: string, excludeId?: number): boolean => {
  ...
  const validateForm = (): boolean => {
    ...
    if (isDuplicateCategory(trimmedName, excludeId)) {
    ...
    try {
      ...
      if (err.response?.status === 400 && 
          (err.response?.data?.detail?.toLowerCase().includes('already exists') ||
           err.response?.data?.name?.includes('already exists'))) {
        setValidationError('A category with this name already exists')
      } else {
        ...
        <div className="bg-white rounded-lg max-w-md w-full">
          ...
          <form onSubmit={handleSubmitCategory} className="p-6 space-y-4">
            <div>
              ...
              {/* Real-time duplicate check display */}
              {formData.name.trim() && !validationError && (
                <div className="mt-2 text-sm text-gray-500">
                  {isDuplicateCategory(formData.name.trim(), editingCategory?.id) ? (
                    <div className="flex items-center gap-2 text-amber-600">
                      <AlertCircle className="w-4 h-4" />
                      <span>This category name already exists</span>
                    </div>
                  ) : (
    ...
    return (
      <div className="p-6">
        ...
        <button 
          ...
          className="mt-4 px-4 py-2 bg-purple-500 text-white rounded hover:bg-purple-600 block mx-auto"
          type="button"
        >
          Retry
        </button>
